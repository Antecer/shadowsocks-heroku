...
var server = net.createServer(function(connection) {
  ...
  connection.on('data', function(data) {
    ...
    if (stage === 0) {
      const tempBuf = new Buffer.allocUnsafe(2);
      tempBuf.write('\u0005\u0000', 0);
    ...
    if (stage === 1) {
      try {
        ...
        if (cmd !== 1) {
          ...
          const reply = new Buffer.allocUnsafe('\u0005\u0007\u0000\u0001', 'binary');
        ...
        let buf = new Buffer.allocUnsafe(10);
        ...
        if (data.length > headerLength) {
          buf = new Buffer.allocUnsafe(data.length - headerLength);
